...
const Tooltip = ({
  ...
  return createPortal(
    ...
    >
      <motion.div
        ref={ref}
        className="whitespace-nowrap rounded-lg bg-oklch(0.145 0 0) px-2 py-1 text-[11px] font-medium text-oklch(1 0 0) shadow-md dark:bg-oklch(0.985 0 0) dark:text-oklch(0.145 0 0)"
      ...
      {showMonths && (
        ...
        >
          {toMonthLabels(visible).map((month, index) => (
            ...
            >
              {month && (
                <span className="absolute left-0 top-0 text-[10px] leading-none text-oklch(0.145 0 0)/40 dark:text-oklch(0.985 0 0)/40">
                  {month}
                </span>
              )}
            </div>
          ...
          <div key={weekIndex} className="flex flex-col" style={{ gap }}>
            {week.map((day) => (
              <motion.div
                ...
                className="shrink-0 rounded-[3px] bg-oklch(0.145 0 0)/[0.08] dark:bg-oklch(0.985 0 0)/[0.08]"
...
const Avatar = ({
  ...
  <motion.span
    ...
    transition={transition}
    className={cn(
      "grid size-7 shrink-0 place-items-center overflow-hidden rounded-full bg-neutral-200 text-[11px] font-medium uppercase text-oklch(0.145 0 0)/70 ring-2 ring-oklch(1 0 0) dark:bg-neutral-800 dark:text-oklch(0.985 0 0)/70 dark:ring-oklch(0.145 0 0)",
...
const RepoRow = ({
  ...
  const className =
    "flex items-center gap-3 rounded-xl mx-2 px-2 py-2 transition-colors hover:bg-oklch(0.145 0 0)/5 dark:hover:bg-oklch(0.985 0 0)/5";

  const content = (
    <>
      <Avatar repo={repo} layoutId={layoutId} transition={transition} />
      <span className="flex-1 truncate text-sm text-oklch(0.145 0 0) dark:text-oklch(0.985 0 0)">
        {repo.name}
      </span>
      <span className="text-sm tabular-nums text-oklch(0.145 0 0)/70 dark:text-oklch(0.985 0 0)/70">
        {repo.count}
      </span>
    </>
  );
...
const GitHubActivity = ({
  ...
  return (
    <div
      ...
      {...props}
    >
      <p className="mb-4 text-base font-medium text-oklch(0.145 0 0) px-1.5 dark:text-oklch(0.985 0 0)">
        {heading}
      </p>
      ...
      {repos.length > 0 && (
        <motion.div
          ...
          className={cn(
            "absolute inset-x-3 bottom-3 overflow-hidden bg-oklch(1 0 0)/90 backdrop-blur-xl dark:bg-oklch(0.205 0 0)/90",
            open && "top-3",
          )}
        ...
        >
          ...
          >
            <span className="truncate text-sm text-oklch(0.145 0 0) dark:text-oklch(0.985 0 0)">{label}</span>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="grid size-7 shrink-0 place-items-center rounded-full bg-oklch(1 0 0) dark:bg-oklch(0.205 0 0)"
              >
